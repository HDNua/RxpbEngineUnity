...
public class TiledGeometryScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector3 localScale = Map.Instance.LocalScale;
        ...
        {
            InitFlatGround(scaleX, scaleY, material);
        }
        // 기울어진 지형 충돌체의 처리입니다.
        else if (_slopeGroundCollider != null)
        {
            InitSlopeGround(material);
        }
        // 그 외의 경우 예외 처리합니다.
        else
        {
            throw new Exception
                ("TiledGeometryScript가 FlatGround 또는 SlopeGround 둘 중 어느 것도 아닙니다.");
        }
    }
    /// <summary>
    ...
    /// 평면을 초기화합니다.
    /// </summary>
    /// <param name="scaleX">가로 방향 스케일입니다.</param>
    /// <param name="scaleY">세로 방향 스케일입니다.</param>
    /// <param name="material">면의 성질입니다.</param>
    void InitFlatGround(float scaleX, float scaleY, PhysicsMaterial2D material)
    {
        // 
    ...
    /// 경사면 엔티티를 초기화합니다.
    /// </summary>
    /// <param name="material">경사면의 성질입니다.</param>
    void InitSlopeGround(PhysicsMaterial2D material)
    {
        // 
        EdgeCollider2D groundEdge, leftEdge, rightEdge, bottomEdge;
        ...
        // 스케일을 맞춥니다.
        float mapOriginX = Map.Instance.Position.x; // 
        float mapOriginY = Map.Instance.Position.y; // _database.Map.transform.position.y;
        for (int i = 0, len = points.Length; i < len; ++i)
        {
            points[i].x = points[i].x / transform.localScale.x + mapOriginX;
            points[i].y = points[i].y / transform.localScale.y + mapOriginY;
        }
